...
class UpdateEmail : public FlowBase {
 ...
 private:
  void Schedule(base::TimeDelta delay = base::Seconds(0),
                endpoint_client::RequestHandle current_request = {});

  void Validate(endpoint_client::RequestHandle current_request);

  void OnValidate(endpoints::AuthValidate::Response response);
