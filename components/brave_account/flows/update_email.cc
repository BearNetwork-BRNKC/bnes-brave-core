...
void UpdateEmail::Schedule(base::TimeDelta delay,
                           RequestHandle current_request) {
  timer_.Start(FROM_HERE, delay,
               base::BindOnce(&UpdateEmail::Validate, base::Unretained(this),
                              std::move(current_request)));
}

void UpdateEmail::Validate(RequestHandle current_request) {
  ...
  // `Validate`, which resets it (cancelling any still-pending previous
  // attempt) before issuing the new one.
  current_request = SendCallerOwnedRequest<AuthValidate>(
      std::move(request),
      base::BindOnce(&UpdateEmail::OnValidate, weak_factory_.GetWeakPtr()));

  // Replace normal cadence with the watchdog timer.
  Schedule(kWatchdogInterval, std::move(current_request));
}

void UpdateEmail::OnValidate(AuthValidate::Response response) {
  const auto email =
      response.body
  ...
  Schedule(kAuthValidatePollInterval);
}
