...
class BravePasswordImporterTest : public testing::Test {
 protected:
  ...
  // profile directory, encrypting with the same OSCryptAsync instance the
  // importer will later decrypt with.
  void WriteLoginData(
    ...
    ASSERT_TRUE(database.Init(base::NullCallback(), GetEncryptorSync()));
  ...
  size_t RunImport() {
    ...
    importer.Start(source_path_, profile_.get(), future.GetCallback());
    return future.Get();
  }
...
TEST_F(BravePasswordImporterTest, MissingLoginDataReportsZero) {
