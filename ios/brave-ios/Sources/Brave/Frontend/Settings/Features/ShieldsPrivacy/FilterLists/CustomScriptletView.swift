...
struct CustomScriptletView: View {
  ...
  init(customScriptlet: CustomScriptlet? = nil) {
    ...
    if let customName = customScriptlet?.name {
      name = String(customName.deletingPathExtension.trimmingPrefix("user-"))
    }
  ...
  var body: some View {
      ...
      Section(
        content: {
          HStack(alignment: .firstTextBaseline, spacing: 0) {
            ...
            TextField("", text: $customScriptletName, prompt: Text("my-scriptlet"))
        ...
        footer: {
          Text(
            "The name must start with “\(CustomScriptlet.namePrefix)” and end with “\(CustomScriptlet.nameExtension)”"
          )
        }
      )
      Section(
    ...
    .navigationTitle(Text(editingScriptlet == nil ? "Add New Scriptlet" : "Edit Scriptlet"))
